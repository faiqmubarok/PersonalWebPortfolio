...
const Resume = () => {
  ...
  const experience = [
    {year: 'Maret 2022 - June 2024', company: 'UKost', position: 'Frontend Developer'},
    {year: 'Februari 2024', company: 'Kopidia', position: 'UI/UX Designer'},
  ]

  const organization = [
    {name: 'Dies Natalis Sistem Informasi 7', position: 'Person Of Charge', period: 'Sep 2023 - Dec 2023'},
    ...
    {name: 'Romadhon Fun With Ankasa-HK', position: 'Secretary', period: 'Mei 2020'},
  ]
  ...
  return (
    <>
          ...
          <div className="flex flex-col gap-5 w-full">
            ...
            <ul className="flex flex-col gap-4 lg:gap-3">
              {education.map((item, index) => (
                <li key={index} className={`flex flex-col gap-4 p-3 rounded-lg font-poppins justify-evenly items-start py-4 dark:bg-transparent outline outline-1  dark:text-darkSecondary outline-white dark:outline-[#353535]  ${index % 2 == 0 ? 'bg-[#EEF5FA] ' : 'bg-[#FFF3F4]'}`}>
          ...
          <div className="flex flex-col gap-5 w-full">
            ...
            <ul className="flex flex-col gap-4 lg:gap-3">
                {experience.map((item, index) => (
                  <li key={index} className={`flex flex-col gap-4 p-3 rounded-lg font-poppins justify-evenly items-start py-4 dark:bg-transparent outline outline-1  dark:text-darkSecondary outline-white dark:outline-[#353535]  ${index % 2 == 0 ? 'bg-[#FFF3F4] ' : 'bg-[#EEF5FA]'}`}
                  >
                    <p className="text-xs">{item.year}</p>
      ...
      <section className="w-full bg-lightPrimary dark:bg-darkPrimary px-6 md:px-8 lg:px-14 py-8 md:py-10 lg:py-14 shadow-md">
            ...
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                  {organization.map((item, index) => (
                    <tr key={index} className="bg-white/30 backdrop-blur-sm border-b  dark:border-gray-700 text-sm">
                      <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        {item.name} {/* Ganti dengan nama organisasi */}
                      </td>
                      <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        {item.position} {/* Ganti dengan posisi */}
                      </td>
                      <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        {item.period} {/* Ganti dengan periode */}
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <section className="px-6 md:px-8 lg:px-14 py-8 md:py-10 lg:py-14">
        <h3 className="text-xl font-bold font-poppins text-black dark:text-white mb-6">Certification</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {certification.map((item, index) => (
            <button 
            onClick={() => handleThumbnailClick(item.image)}
            key={index}
            className=" lg:border-none border-b border-lightSecondary dark:border-darkSecondary py-3"
            >
