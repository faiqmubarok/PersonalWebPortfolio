...
const Profile = () => {
  ...
  const socialMedia = [
    { name : "whatsapp", icon : <FaWhatsapp className="text-[#25D366] w-6 h-6 group-hover:text-white" />, url: 'https://api.whatsapp.com/send/?phone=6282117748606&text&type=phone_number&app_absent=0'},
    { name : "instagram", icon : <FaInstagram className="text-[#DA4A80] w-6 h-6 group-hover:text-white" />, url: 'https://www.instagram.com/faiqmubarok._/'},
    { name : "linkedin", icon : <FaLinkedin className="text-[#0077B5] w-6 h-6 group-hover:text-white" />, url: 'https://www.linkedin.com/in/faiq-mubarok-35a11a229/'},
    { name : "github", icon : <FaGithub className="text-[#24292F] dark:text-white w-6 h-6 group-hover:text-white" />, url: 'https://github.com/faiqmubarok'},
  ];

  const info = [
    { name : 'phone', title : 'Phone', icon: <SlScreenSmartphone className="text-[#667BC6] w-6 h-6"/>, value : '+62 821 1774 8606'},
    { name : 'email', title : 'Email', icon: <HiOutlineMailOpen className="text-[#50B498] w-6 h-6"/>, value : 'faiqmubarok@gmail.com'},
    { name : 'location', title : 'Location', icon: <IoLocation className="text-[#B692C2] w-6 h-6"/>, value : 'Purwokerto, Indonesia'},
    { name : 'birthday', title : 'Birthday', icon: <LuCalendarDays className="text-[#4F1787] w-6 h-6"/>, value : 'November 20, 2003'},
  ]
            ...
            {info.map((info, index) => (
              <li 
                ...
                className={`flex items-center justify-start gap-3 pb-2 ${
                  index !== info.length - 1 ? "border-b border-darkSecondary border-opacity-40" : ""}`}>
                {/* Icon */}
                ...
                <div className="flex flex-col font-poppins">
                  <span className="text-xs text-darkSecondary text-opacity-80">{info.title}</span>
                  <span className="text-sm font-medium text-black dark:text-white">{info.value}</span>
                </div>
              </li>
            ))}
          </ul>
